...
void mnt4753_libsnark::print_Fr(mnt4753_libsnark::vector_Fr *a, size_t i) { a->data->at(i).print(); }
...
void mnt6753_libsnark::print_Fr(mnt6753_libsnark::vector_Fr *a, size_t i) { a->data->at(i).print(); }
...
void mnt4753_libsnark::calc_wnaf(std::vector<long> &wnaf,
    vector_Fr *scalar_start, size_t length) {
    auto start = scalar_start->data->begin();
    auto end = start + length;
    for (auto scalar_it = start; scalar_it != end; scalar_it++) {
        std::vector<long> w = libff::find_wnaf(4, scalar_it->as_bigint());
        wnaf.insert(wnaf.end(), w.begin(), w.end());
    }
}

void mnt6753_libsnark::calc_wnaf(std::vector<long> &wnaf,
    vector_Fr *scalar_start, size_t length) {
    auto start = scalar_start->data->begin();
    auto end = start + length;
    for (auto scalar_it = start; scalar_it != end; scalar_it++) {
