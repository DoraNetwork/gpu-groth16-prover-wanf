...
template<typename ppT>
void output_g1_wnaf_multiples(int C, const std::vector<G1<ppT>> &vec, FILE *output) {
    ...
    //     5P0, ...,    5Pn,
    //          ...,
    //  2^(C-1)-1 P0, ..., 2^(C-1)-1 Pn,
    //  -2^(C-1)+1 P0,..., -2^(C-1)+1 Pn]
    std::vector<G1<ppT>> multiples;
    size_t len = vec.size();
    multiples.resize(len * (1U << C));
    std::copy(vec.begin(), vec.end(), multiples.begin());

    std::vector<G1<ppT>> double_vec;
    double_vec.resize(len);
    for (size_t i = 0; i < len; i++) {
        multiples[len + i] = -multiples[i];
        double_vec[i] = vec[i] + vec[i];
    }

    for (size_t i = 2; i < (1U << C); i += 2) {
        size_t prev_row_offset = (i-2)*len;
        size_t curr_row_offset = i*len;
#ifdef MULTICORE
#pragma omp parallel for
#endif
        for (size_t j = 0; j < len; j++) {
           multiples[curr_row_offset + j] = double_vec[j] + multiples[prev_row_offset + j];
           multiples[curr_row_offset + len + j] = -multiples[curr_row_offset + j];
        }
    }

    if (multiples.size() != (1U << C)*len) {
        fprintf(stderr, "Broken preprocessing table: got %zu, expected %zu\n",
                multiples.size(), (1U << C) * len);
        abort();
    }
...
template<typename ppT>
void output_g2_wnaf_multiples(int C, const std::vector<G2<ppT>> &vec, FILE *output) {
    // If vec = [P0, ..., Pn], then multiples holds an array
    //
    // [    P0, ...,     Pn,
    //     -P0, ...,    -Pn,
...
#else
    FILE *output = fopen(output_path, "w");
    printf("Processing B1 wnaf...\n");
    output_g1_wnaf_multiples<ppT>(C, params.B1, output);
    printf("Processing B2 wnaf...\n");
    output_g2_wnaf_multiples<ppT>(C, params.B2, output);
    printf("Processing L wnaf...\n");
    output_g1_wnaf_multiples<ppT>(C, params.L, output);
    fclose(output);
#endif
}
